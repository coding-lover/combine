        ...
        <el-col :span="24" :offset="0" class="wrap">
            <el-tabs v-model="activeName" @tab-click="handleClick">
                ...
                <el-tab-pane name="first" class="wrap-padding">
                    <span slot="label">程式替换<el-badge :value="totalNum" class="large-btn"
                            :hidden="totalNum > 0 ? false : true"></el-badge></span>

                    <el-col :span="12" class="margin-bottom-10" v-for="(item, idx) in nodeList">
                        <el-card class="box-card">
                            <div slot="header" class="clearfix">
                                <el-col :span="5">
                                    <el-input v-model="item.search" prefix-icon="el-icon-search"
                                        placeholder="查找"></el-input>
                                </el-col>
                                <el-col :span="5">
                                    <el-input v-model="item.replace" prefix-icon="el-icon-edit"
                                        placeholder="替换"></el-input>
                                </el-col>
                                <el-badge :value="item.fileNum" class="" :hidden="item.fileNum > 0 ? false : true">
                                    ...
                                    <el-button v-show="replaceFlag(idx)" class="ml-3" size="small"
                                        type="success">已替换</el-button>
                                </el-badge>
                                <el-button size="small" type="primary" class="margin-left-3"
                                    @click="downloadFile(idx)">下载</el-button>
                                <el-button size="small" type="danger" class="margin-left-3"
                                    @click="clearSearchReplace(idx)">清空</el-button>
                                <el-button size="small" type="success" class="ml-3"
                                    @click="getReplaceHis(idx)">替换历史</el-button>

                            </div>
                            <div class="content-overflow">
                                <el-upload class="sr-upload-demo" drag :ref="createUploadKey(idx)" action=""
                                    :on-preview="handlePreview" :on-remove="handleRemove(idx)"
                                    :on-progress="handleProgress" :on-change="myHandleChange(idx)" :auto-upload="false"
                                    :before-upload="handleBeforUpload" multiple>
                                    ...
                                    <div slot="file" slot-scope="{file}">
                                        ...
                                        <i class="el-icon-close" style="width: 20px;"
                                            @click="closeUploadedFile(idx, file, true)"
                                            @mouseover="handleMouseOver(file)"></i>
                    ...
                    <el-col :span="12" class="margin-bottom-10">
                        ...
                        <el-row class="mt-10">
                            <el-badge :value="totalNum" class="large-btn" :hidden="totalNum > 0 ? false : true">
                                <el-button type="danger" size="medium" class="large-btn"
                                    @click="batchReplace">批量替换</el-button>
                            </el-badge>
                        </el-row>

                        <el-row class="mt-10">
                            <el-button type="primary" size="medium" class="large-btn"
                ...
                <el-tab-pane label="程式串联" name="third" style="margin-top: 10px;" class="third-box">
                        ...
                        <el-card class="box-card" shadow="never">
                            ...
                            <div class="content-overflow">
                                <el-upload v-show="!form.delivery" class="upload-demo" drag
                                    :ref="createUploadKey(specialBoxKey)" action=""
                                    :on-remove="handleRemove(specialBoxKey)" :on-change="myHandleChange(specialBoxKey)"
                                    :before-upload="handleBeforUpload" :auto-upload="false"
                                    :file-list="specialBoxFileList" multiple>
                                    <i class="el-icon-upload" v-show="specialBoxFileList.length == 0"></i>
                                    <div class="el-upload__text" v-show="specialBoxFileList == 0">将文件拖到此处，或<em>点击上传</em>
                                    </div>
                                    <div slot="file" slot-scope="{file}">
                                        <a class="el-upload-list__item-name">
                                            <i class="el-icon-document"></i>
                                            <span class="upload-file-name-fmt">{{ file.name }} </span>
                                            <span style="color:#F56C6C;margin-left: 10px;;">上传耗时：{{ showSpendTime(file)
                                                }}</span>
                                        </a>
                                        <label class="el-upload-list__item-status-label">
                                            <i class="el-icon-upload-success el-icon-circle-check"></i>
                                        ...
                                        <i class="el-icon-close" style="width: 20px;"
                                            @click="closeUploadedFile(specialBoxKey, file, false)"
                                            @mouseover="handleMouseOver(file)"></i>
                                ...
                                <ul class="el-upload-list el-upload-list--text att" v-show="form.delivery">
                                    ...
                                    <li tabindex="0" class="el-upload-list__item is-ready" v-for="file in getFileList">
                                        ...
                                        <el-tooltip class="item" effect="light" placement="right">
                                            ...
                                            <i class="el-icon-close" style="width: 20px;"
                                                @click="deleteNodeListFile(file.nodeIdx, file.id)"
                                                @mouseover="handleMouseOver(file)"></i>
                                        </el-tooltip>
                                    </li>
                                </ul>
                            ...
                            <div class="box-operator clearfix">
                                <el-col :span="12">
                                    <el-badge :value="getCombineFileNum" class="large-btn"
                                        :hidden="getCombineFileNum > 0 ? false : true">
                                        <el-button type="primary" v-show="form.ready" size="medium" class="large-btn"
                                            @click="combineFile">合并</el-button>
                                ...
                                <el-col :span="12"><el-button type="danger" size="medium" class="large-btn"
                                        @click="clearBoxFileList">清空</el-button></el-col>
                            </div>
                    ...
                    <el-col :span="14">
                        <el-form ref="form" :rules="formRules" :model="form" label-width="150px" class="main-conf-form">
                            <el-form-item label="使用替换">
                                ...
                                <span
                                    style="color: #F56C6C;margin-left: 5px;display: inline-block;">使用已经替换的程式或者重新上传程式</span>
                            </el-form-item>

                            <el-form-item label="模板名" prop="templateName">
                                <el-input placeholder="模板名" class="margin-bottom-10"
                                        v-model="form.templateName"></el-input>
                            </el-form-item>

                            <el-form-item label="文件头删除行数" prop="fileHeaderDeletedLine">
                                <el-select v-model="form.fileHeaderDeletedLine" placeholder="请选择">
                                    <el-option v-for="num in 20" :key="num" :label="num" :value="num">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                            ...
                            <el-form-item label="文件尾增加" prop="fileFooter" class="clearfix">
                                <el-input type="textarea" v-model="form.fileFooter"
                        ...
                        <el-form>
                            <div v-for="(item, key) in getTemplate">
                                <el-form-item label="" prop="fileName">
                                    <el-col :span="23">
                                        <el-radio v-model="form.selectedTemplate" :label="key" border
                                            class="s-radio clearfix">

                                            <el-form label-width="150px" class="template-form">
                                                <el-form-item label="模板名" prop="templateName">
                                                        <el-input placeholder="模板名" class=""
                                                            v-model="item.templateName"></el-input>
                                                </el-form-item>

                                                <el-form-item label="文件头删除行数" prop="fileHeaderDeletedLine">
                                                        <el-select v-model="item.fileHeaderDeletedLine"
                                                            placeholder="请选择">
                                                            <el-option v-for="num in 20" :key="num" :label="num"
                                                                :value="num">
                                                            </el-option>
                                                        </el-select>
                                                </el-form-item>
                                                ...
                                                <el-form-item label="文件尾增加" prop="fileFooter" class="clearfix">
                                                        <el-input type="textarea" v-model="item.fileFooter"
                                                            :autosize="{ minRows: 3 }"></el-input>
                                                </el-form-item>

                                                <el-form-item label="串联程式新名" prop="fileName">
                                                    <el-input placeholder="串联程式新名" v-model="item.fileName"
                                                        class="input-with-select">
                                                        <el-select v-model="item.fileSuffix" slot="append"
                                                            placeholder="请选择" prop="fileSuffix">
                                                            <el-option v-for="SuffixItem in form.fileSuffixOptions"
                                                ...
                                                <el-form-item label="文件保存路径" prop="filePath">
                                                    <el-input placeholder="文件保存路径" v-model="item.filePath"
                                                        class="input-with-select mt-10" readonly>
                                                        <el-button slot="append" icon="el-icon-folder" type="success"
                                                            @click="openFolder(key)"></el-button>
                                                    </el-input>
                                                </el-form-item>
                    ...
                    <el-dialog title="选择保存的目录" :visible.sync="dialogFolderSelectVisible">
                        ...
                        <div class="folder-content">
                            <el-cascader v-model="folderValue" :options="folderOptions" :props="folderProps" clearable
                                @change="handleChange">
                ...
                <el-tab-pane label="串联历史" name="history" style="margin-top: 10px;" class="history-box">
                    <span slot="label">串联历史<el-badge :value="combineHisNum" class="large-btn"
                            :hidden="combineHisNum > 0 ? false : true"></el-badge></span>
                    <el-empty v-if="combineHisNum == 0" description="没有历史记录"></el-empty>
                    <el-card v-if="combineHisNum > 0" class="box-card wrap-card">
                        <div slot="header" class="clearfix">
                            <el-button style="float: right;cursor: pointer;" type="danger"
                                @click="delAllHis()">全部删除</el-button>
                        </div>
                        <div class="text item">
                            <el-card class="box-card mt-10" v-for="(his, key) in getCombineHisList" shadow="hover">
                                <div slot="header" class="clearfix cb-his-card-header">
                                    ...
                                    <el-button style="float: right; padding: 3px 0; color:red;cursor: pointer;"
                                        type="text" @click="delHis(key)">删除</el-button>
                                </div>
        ...
        return {
            ...
            folderProps: {
                ...
                lazyLoad: (node, resolve) => {
                    ...
                    node.path = path.join('/');
                    let a = 11;
                    this.readFolderAndResolve(node, resolve);
                    // setTimeout(() => {
    ...
    methods: {
        ...
        dialogSaveFolder() {
            ...
            if(this.operatorTemplateIdx == -1) {
                ...
                this.form.templateList[this.operatorTemplateIdx]['filePath'] = this.selectedFolder.join('/');
            }
        ...
        openFolder(idx) {
            ...
            if(idx == -1) {
                this.folderValue = this.selectedFolder;
            } else {
                this.folderValue = this.form.templateList[idx]['filePath'].length > 0 ? this.form.templateList[idx]['filePath'].split('/') : [];
            }
        },
        basename(str) {
        ...
        readFolderAndResolve(node, resolve) {
            return this.wmcReadDir(node.path).then(res => {
                //debugger
                let subNode = [];
                res.map(item => {
                    if (item.is_dir) {
                        let basename = this.basename(item.path);
                        subNode.push({ value: basename, label: basename });
                    }
                });
        ...
        selectTemplate(idx) {
            ...
            this.form.fileHeader = cur.fileHeader;
            this.form.fileFooter = cur.fileFooter;
            this.form.fileName = cur.fileName;
            this.form.filePath = cur.filePath;
            this.form.templateName = cur.templateName;

            this.form.fileHeaderDeletedLine = cur.fileHeaderDeletedLine;
            this.form.fileFooterDeletedLine = cur.fileFooterDeletedLine;

            this.form.fileSuffix = cur.fileSuffix;

            this.selectedFolder = cur.filePath.split('/');
        },
        ...
        saveTemplate() {
            //debugger
            let template = JSON.parse(JSON.stringify(this.form));
            template.templateList = [];
            //template.value = this.form.fileFooter;
            //template.name = this.form.templateName;
            //debugger
            if (!template.fileFooter) {
            ...
            for (let idx in this.form.templateList) {
                if (this.form.templateList[idx].templateName == template.templateName) {
                    this.form.templateList[idx] = template;
...
.main-conf-form .input-with-select .el-input {
    width: 100px;
}


.main-conf-form .input-with-select .el-button {
    width: 100px;
}
...
.third-box .s-radio .el-radio__label:before,
...
.template-form .el-textarea__inner {
    width: 100%;
}


.template-form .min-width-256 {
    min-width: 256px;
}

.template-form .input-with-select .el-input--suffix {
    width: 80px;
}


.template-form .input-with-select .el-button {
    width: 80px;
}
